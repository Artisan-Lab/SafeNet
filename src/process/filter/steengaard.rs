        ...
        -> HashMap<String,(i32,bool,bool)>
{
    ...
    for var_target in variable_set{
        ...
        if let Some(target_get_value) = alias_map.get(var_target){
            let now_value = (target_get_value.0,target_get_value.1,target_get_value.2);
            new_alias_map_after_union.insert(var_target.to_string(), now_value);
        }
